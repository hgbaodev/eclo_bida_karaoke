...
export default function JobUpgradeStorage({ className }: { className?: string }) {
  return (
    <div className={className}>
      <div className="flex items-center justify-between rounded-lg bg-gray-100 p-5 @xl:p-7">
        <div className="shrink-0">
          ...
          <Link href="#" className="flex items-center gap-2 font-bold uppercase text-primary">
            <PiArrowCircleUpFill className="size-6" /> Upgrade Now
          </Link>
        </div>
        <div>
          <Image width={150} height={150} src={UpStorageImg} alt="Upgrade Storage" sizes="(max-width: 768px) 100vw" />
        </div>
      </div>
    </div>
  );
