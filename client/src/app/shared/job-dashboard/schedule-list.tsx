...
import { PiArrowRight, PiArrowLeft, PiPlusBold, PiClock, PiDotsThreeVerticalBold } from 'react-icons/pi';
  ...
  return (
    ...
    >
      <div className="border-r border-gray-200 pt-5 lg:pt-7 3xl:border-r-0">
        <Title as="h3" className="px-5 pb-4 font-inter text-base font-semibold text-gray-800 sm:text-lg lg:px-7">
          Schedule Calendar
        </Title>
        <Calendar
          prev2Label={false}
      ...
      <div className="flex flex-col p-5 @[90rem]:grow lg:p-7">
        <div className="@[90rem]:grow">
          <div className="flex items-center justify-between">
            <Title as="h3" className="font-inter text-base font-semibold text-gray-800 sm:text-lg">
              Schedule list
            </Title>
  ...
  return (
    <div className="flex items-center gap-4">
      <div className="w-14 shrink-0 space-y-0.5 rounded-xl px-3 py-2.5 text-center text-sm shadow dark:shadow-gray-300">
        <span className="block font-medium uppercase text-red-dark">{data?.day}</span>
        <span className="block font-semibold leading-none text-gray-900">{data?.date}</span>
      </div>
      <div className="text-xs @md:text-sm">
        <span className="line-clamp-1 block font-semibold text-gray-900 dark:text-gray-700">{data?.title}</span>
